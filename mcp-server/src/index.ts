...
server.registerTool(
  ...
  async ({ answers }) => ({
    content: [{ type: 'text', text: jsonText(runMatch(answers as FormAnswers)) }],
  }),
);
...
server.registerTool(
  'list_vendors',
  {
    description: 'List vendors in the PickTheRobot catalog for a category, optionally filtered by region (US, EU, UK, APAC).',
    inputSchema: {
      category: categorySchema,
...
server.registerTool(
  'get_price_bands',
  {
    description: 'Indicative USD purchase and RaaS price bands shared across PickTheRobot guides (not vendor quotes).',
    inputSchema: {},
  },
  async () => ({
...
server.registerTool(
  'list_comparisons',
  {
    description: 'List comparison / decision guide pages (AMR vs AGV, lease vs buy, robot vs staff, etc.).',
    inputSchema: {},
  },
  async () => ({
